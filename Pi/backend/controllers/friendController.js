...
module.exports.getSuggestions = async (req, res) => {
  ...
  if(UserTok==null){
   ...
   let userConnected=await userModel.findById(req.params.idUser)
  const myrequests = [];
  if (userConnected.typeUser!="admin"){
   users = await userModel.find({ _id: { $ne: req.params.idUser},typeUser:  { $ne: "admin"} });}else
  { users = await userModel.find({ _id: { $ne: req.params.idUser} });}
